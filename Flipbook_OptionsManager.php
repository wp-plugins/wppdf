...
class Flipbook_OptionsManager {
    ...
    public function settingsPage() {
        if (!current_user_can('manage_options')) {
            wp_die(__('You do not have sufficient permissions to access this page.', 'flipbook'));
        }
        ...
        ?>
        <div class="wrap">
            <h2><?php _e('System Settings', 'flipbook'); ?></h2>
            <table class="form-table"><tbody>
            <tr><td><?php _e('System', 'flipbook'); ?></td><td><?php echo php_uname(); ?></td></tr>
            <tr><td><?php _e('PHP Version', 'flipbook'); ?></td>
                <td><?php echo phpversion(); ?>
                <?php
                if (version_compare('5.2', phpversion()) > 0) {
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    _e('(WARNING: This plugin may not work properly with versions earlier than PHP 5.2)', 'flipbook');
                    echo '</span>';
                }
                ?>
                </td>
            </tr>
            <tr><td><?php _e('MySQL Version', 'flipbook'); ?></td>
                <td><?php echo $this->getMySqlVersion() ?>
                    <?php
                    echo '&nbsp;&nbsp;&nbsp;<span style="background-color: #ffcc00;">';
                    if (version_compare('5.0', $this->getMySqlVersion()) > 0) {
                        _e('(WARNING: This plugin may not work properly with versions earlier than MySQL 5.0)', 'flipbook');
                    }
                    echo '</span>';
                    ?>
                </td>
            ...
            <h2><?php echo $this->getPluginDisplayName(); echo ' '; _e('Settings', 'flipbook'); ?></h2>
            ...
            <?php settings_fields($settingsGroup); ?>
                ...
                <p class="submit">
                    <input type="submit" class="button-primary"
                           value="<?php _e('Save Changes', 'flipbook') ?>"/>
                </p>
            </form>
        </div>
        <?php
    ...
    protected function getOptionValueI18nString($optionValue) {
        switch ($optionValue) {
            case 'true':
                return __('true', 'flipbook');
            case 'false':
                return __('false', 'flipbook');

            case 'Administrator':
                return __('Administrator', 'flipbook');
            case 'Editor':
                return __('Editor', 'flipbook');
            case 'Author':
                return __('Author', 'flipbook');
            case 'Contributor':
                return __('Contributor', 'flipbook');
            case 'Subscriber':
                return __('Subscriber', 'flipbook');
            case 'Anyone':
                return __('Anyone', 'flipbook');
        }
        return $optionValue;
    }
